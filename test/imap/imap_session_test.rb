...
class ImapSessionTest < Minitest::Test
  ...
  # 30-second waits. Settings-backed tunables have no constant normally;
  # defining one on the server class is their documented override seam.
  def with_imap_const(name, value)
    klass = MailOnRails::ImapServer
    had_old = klass.const_defined?(name, false)
    old = klass.const_get(name) if had_old
    klass.send(:remove_const, name) if had_old
    klass.const_set(name, value)
    yield
  ensure
    klass.send(:remove_const, name)
    klass.const_set(name, old) if had_old
  end
  ...
  def test_id_reports_no_server_identity
    with_session do |client|
      client.gets("\r\n")
      response = command(client, "z1", %(ID ("name" "test-client" "version" "1.0")))
      assert_match(/\A\* ID NIL\r\nz1 OK/, response)
  ...
  # Authenticated, so the flood exercises the count cap's refuse-and-drain
  # path (pre-auth the stricter MAX_PREAUTH_LITERAL_BYTES cap hangs up
  # instead - preauth_literal_test.rb).
  def test_chained_literal_flood_is_refused_before_exhausting_memory
    with_imap_const(:MAX_COMMAND_LITERALS, 3) do
      with_session do |client|
        client.gets("\r\n") # greeting
        command(client, "a0", "LOGIN #{EMAIL} #{PASSWORD}")
  ...
  # are refused in aggregate, not accepted one by one. Authenticated, like
  # the count-cap test above.
  def test_aggregate_literal_octets_are_capped
    with_max_literal(100) do
      with_session do |client|
        client.gets("\r\n") # greeting
        command(client, "a0", "LOGIN #{EMAIL} #{PASSWORD}")
        client.write("a1 X {60+}\r\n#{"A" * 60}{60+}\r\n#{"B" * 60}\r\n")
        assert_match(/\Aa1 NO \[TOOBIG\]/, read_until_tagged(client, "a1"))
        assert_match(/\Aa2 OK/, command(client, "a2", "NOOP"))
        command(client, "a3", "LOGOUT")
      end
