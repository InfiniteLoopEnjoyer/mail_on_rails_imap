...
class ListStatusSizeTest < Minitest::Test
  ...
  # The exact shape iOS Mail (maild) opens every account sync with; the
  # SPECIAL-USE return option (RFC 6154 §4) must not reject the LIST -
  # rejecting it leaves the client with no folder list at all.
  test "list return accepts the special-use option alongside status" do
    c = connect
    reply = command(c, "l1", %(LIST "" "*" RETURN (STATUS (MESSAGES UIDNEXT UIDVALIDITY UNSEEN HIGHESTMODSEQ) SPECIAL-USE)))
    assert_match(/\* LIST \([^)]*\\Sent\) "\/" "Sent"/, reply)
    assert_match(/\* STATUS "Sent" \(MESSAGES 0 /, reply)
    assert_match(/^l1 OK/, reply)

    reply = command(c, "l2", %(LIST "" "*" RETURN (SPECIAL-USE)))
    assert_match(/\* LIST \([^)]*\\Trash\) "\/" "Trash"/, reply)
    refute_match(/STATUS/, reply)
    assert_match(/^l2 OK/, reply)
  end

  test "special-use selection option limits the listing" do
    c = connect
    reply = command(c, "s1", %(LIST (SPECIAL-USE) "" "*"))
    %w[Sent Drafts Trash Junk].each { |name| assert_match(/\* LIST \([^)]*\) "\/" "#{name}"/, reply) }
    refute_match(/"INBOX"/, reply)
    assert_match(/^s1 OK/, reply)

    assert_match(/\As2 BAD/, command(c, "s2", %(LIST (BOGUS) "" "*")))
  end
