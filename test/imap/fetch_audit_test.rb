...
class FetchAuditTest < Minitest::Test
  ...
  test "fetch syntax errors are bad" do
    ...
    [
      ...
      "FETCH 1 BODY[HEADER.FIELDS  (DATE)]",  # single SP before the list (canonical grammar)
      "FETCH 1 BODY[HEADER.FIELDS ( DATE)]",  # no leading space inside the list
      "FETCH 1 BODY[HEADER.FIELDS ((DATE))]", # no nested parens in the list
  ...
  # FETCH items can arrive as literals, which may carry any byte. A CR/LF
  # smuggled into a HEADER.FIELDS list used to satisfy the /m section
  # grammar and be echoed verbatim on the untagged FETCH line - an
  # authenticated client forging extra untagged responses on its own
  # session. Control bytes in a fetch item are BAD outright.
  test "a literal-carried fetch item with CRLF in the header list is bad" do
    c = select_with_example
    item = "BODY[HEADER.FIELDS (X\r\nY: forged)]"
    c.write("i1 FETCH 1 {#{item.bytesize}+}\r\n#{item}\r\n")

    reply = read_until_tagged(c, "i1")
    assert_match(/\Ai1 BAD/, reply, "a control byte in a fetch section must be refused, not echoed")
  end

  test "a literal-carried valid fetch item still works" do
    c = select_with_example
    item = "BODY.PEEK[HEADER.FIELDS (SUBJECT)]"
    c.write("i2 FETCH 1 {#{item.bytesize}+}\r\n#{item}\r\n")

    reply = read_until_tagged(c, "i2")
    assert_match(/afternoon meeting/, reply)
    assert_match(/\Ai2 OK/, reply.lines.last)
  end
